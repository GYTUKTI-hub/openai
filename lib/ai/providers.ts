diff -ru /home/claude/repo2/openai-main/lib/ai/llm.ts /home/claude/repo/openai-main/lib/ai/llm.ts
--- /home/claude/repo2/openai-main/lib/ai/llm.ts	2026-09-24 15:15:47.000000000 +0000
+++ /home/claude/repo/openai-main/lib/ai/llm.ts	2026-09-26 06:23:16.613271673 +0000
@@ -322,6 +322,27 @@
  ...
  */
+/**
+ * Delay before a `callLLM` retry attempt: exponential backoff with a small
+ * jitter, capped at `maxDelayMs`. Mirrors the formula used by the generation
+ * package's own retry helper so both layers back off the same way.
+ *
+ * Retrying instantly (no delay) was the previous behaviour here — harmless
+ * for a validation failure, but actively counterproductive for a transient
+ * provider error (rate limit, momentary 5xx): hammering the same endpoint
+ * immediately just reproduces the failure. A brief, growing wait gives the
+ * provider a chance to recover before the next attempt.
+ */
+function llmRetryDelayMs(attempt: number, baseDelayMs = 500, maxDelayMs = 8000): number {
...
+          `[${source}] Validation failed (attempt ${attempt}/${maxAttempts}), ${willRetry ? 'retrying...' : 'giving up'}`,
         );
         lastResult = result;
+        if (willRetry) await sleep(llmRetryDelayMs(attempt));
         continue;
       }
...
diff -ru /home/claude/repo2/openai-main/packages/@openmaic/generation/src/scene-generator.ts /home/claude/repo/openai-main/packages/@openmaic/generation/src/scene-generator.ts
--- /home/claude/repo2/openai-main/packages/@openmaic/generation/src/scene-generator.ts	2026-09-24 15:15:47.000000000 +0000
+++ /home/claude/repo/openai-main/packages/@openmaic/generation/src/scene-generator.ts	2026-09-26 06:22:23.093363373 +0000
@@ -55,7 +55,7 @@
   AICallFn,
